...
`-rwxr-x--x` 를 3자리씩 끊는다: **타입 / 소유자(u) / 그룹(g) / 기타(o)**.

| 구간 | 값 | 8진수 | 의미 |
|---|---|---|---|
| 타입 | `-` | | 일반 파일 (`d`=디렉터리, `l`=링크, `c`/`b`=장치) |
| 소유자 | `rwx` | 7 | 읽기·쓰기·실행 모두 가능 |
| 그룹 | `r-x` | 5 | 읽기·실행 가능, **쓰기 불가** |
| 기타 | `--x` | 1 | **실행만** 가능, 읽기·쓰기 불가 |

→ `-rwxr-x--x` = **751**. `r=4, w=2, x=1` 합.
- 디렉터리에서 `x` = 진입(cd)/내부 접근 권한, `r` = 목록 조회, `w` = 파일 생성·삭제.
