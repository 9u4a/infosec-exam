...
| **Ctime** (Change) | 속성 변경 시간 | 권한·소유자 등 메타데이터 변경 시 |

`stat <파일>` 로 3개 모두 조회. 침해사고 분석 시 보통 **Mtime** 기준으로 특정 시각 이후 변경된(악성) 파일을 찾는다(`find / -newer` 참고). 공격자가 흔적을 지우려 `touch`로 시간을 되돌려도 Ctime(속성 변경)까지는 못 숨기는 경우가 많다.
...
- **기능**
  1. 명령어 해석·실행
  2. 셸 스크립트 프로그래밍(변수·제어문)
  3. 환경 설정(환경변수, 리다이렉션·파이프)
  4. 작업 제어(포그라운드/백그라운드)
