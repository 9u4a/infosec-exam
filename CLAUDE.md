...
- 현재 규모: 노트 65 · 문제 495 (과목별 90/110/130/95/70).
...
⚠ 개인정보보호법은 개정이 잦다(2020 데이터3법 / 2023.9.15 대개정 / 2024.3.15 일부 / 2025.10.31 안전조치 고시 / 2026 시행령). 노트에 조문 번호와 기준 시점(`2026-09 기준`) + 고시번호를 명시한다.
...
```
대표 병기 대상: 위치정보사업(현행 등록제 / 옛 허가제), 과징금(현행 전체매출 3% / 옛 관련매출 3%),
인터넷망 차단(현행 자율 / 옛 100만명 일률), 접속기록 점검(현행 월1회 / 2026.10.30~ 자율).
문제는 원칙적으로 **현행 기준**으로 채점하되 stem에 "(현행 기준)" 등을 표시하고 해설에 변경 이력을 쓴다.

집필 내용은 최종적으로 국가법령정보센터(law.go.kr) 현행 조문·시행처 자료로 교차 확인 필요 — 정답 보증 불가.
