...
```json
{
  ...
  "questions": {
    "5": {
      ...
      "supplement": "선택. 원본 지문(로그·설명문 등)이 누락된 문항의 보충 자료. 마크다운. 문제 본문 아래에 '🧩 지문 재구성 · 원본 데이터 누락분' 으로 표시됨. HTML 태그가 들어가면 코드펜스로 감쌀 것(marked 가 실제 태그로 렌더)."
    }
  }
}
```
...
`CACHE` 버전도 올린다 (현재 **v25**, 실기·CPPG 공용).
...
- 진행 데이터는 기기별 localStorage. 더보기 > 내보내기/가져오기(JSON)로 기기 간 이동. 앱 셸(index.html/style.css/app.js/sw.js) 변경 시 `docs/sw.js` `CACHE` 버전을 올린다 (현재 **v25**).
