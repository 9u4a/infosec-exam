...
- **`notes/반복출제/` 카테고리**: 회차만 바꿔 반복 출제된 유형 정리. `EIS_EXTRA_CATS`(build.mjs)에 등록돼
...
- **`notes/요약/` 카테고리**: 특정 기출에 매이지 않는 종합 요약 노트(현재 1개). `EIS_PIN_CATS`
  (build.mjs, `EIS_EXTRA_CATS`와 대칭)에 등록돼 노트 목록·카테고리 칩 **맨 앞** 고정(기본/취약한
  순/연결 많은 순 모든 정렬에서). 목록 카드는 `📌` 배지 + 좌측 강조선(`.note-item.pinned`)으로
  구분 표시. 노트 상세 페이지는 이 카테고리에 한해 대단원(`##`, 3개 이상)마다 자동 id를 부여하고
  상단에 목차(`.note-toc`)를 렌더 — 클릭 시 `scrollIntoView`로 이동(해시 앵커 아님, 라우터가
  `#/path` 형식만 인식하므로 실제 앵커 링크는 페이지 이동으로 오인됨).
...
- 현재 202문항(단답132/서술44/실무26, 시스템38·네트워크48·앱46·일반26·법규44). 집필/병합은
...
- 앱 `route('mnemonics')`(`#/mnemonics`, 하단 탭 📿): 분류 칩·검색 · **🙈 가리고 암기** 토글(`mnBlind`)
  — 카드/`뜻 보기` 버튼으로 개별 공개. `mnemoCard()`·`mnemoHeroHtml()` 헬퍼. **즐겨찾기**: 문항
  즐겨찾기와 완전 별개인 `store.state.mnemoFavs`(id 배열) — 카드 우상단 ★(`store.toggleMnemoFav`,
  분류 무관 어느 화면에서나 설정 가능) + `⭐ 즐겨찾기만` 칩(`mnFavOnly`, 분류·검색과 AND 결합).
...
앱 셸 파일 변경 시 `docs/sw.js` 의 `CACHE` 버전도 올린다 (현재 **v45**, 실기·CPPG 공용).
...
- **프런트엔드**(`docs/app.js` 의 `SYNC`): 로그인 안 하면 `SYNC.on===false` → 기존과 100% 동일하게
  localStorage 만 사용. 로그인 시 부팅에 `pull` → 이후 `store.save()` 마다 `schedulePush()` 가
  `_dirty=true` 로 표시만 해두고, 실제 push 는 **`PUSH_INTERVAL_MS`(60초) 고정 주기 타이머**가
  dirty 일 때만 수행 — 디바운스(조용해질 때만 묶임)와 달리 활동이 그 이상 간격으로 계속돼도
  주기당 최대 1회로 상한이 걸린다. push 실패 시 `_dirty` 를 그대로 둬 다음 tick 에 재시도.
  오프라인이면 로컬로 동작하다 online(단, 20초 내 재시도는 무시)·visibilitychange 로 재동기화.
  탭을 숨기거나 닫을 때는 `pushNow()` 로 즉시 flush하므로 주기를 늘려도 체감 유실은 없음.
...
- ⚠ 공유 암호라 암호를 아는 사람은 기록을 공유(개인/소그룹용). KV Free 쓰기 1,000/일 —
  60초 고정 주기 push + pull 후 무변경 시 스킵 + 서버측 동일값 스킵의 3중 방어로 여유를 둔다.
...
**회차 반복 풀이 횟수**: `roundAttemptCounts()` 가 `sessions` 중 `scopeLabel` 이 `"N회 M문항"` 형태인
(= 회차별 범위로 제출·중단된) 세션 수를 회차별로 집계 — `#/solve` 회차 선택 드롭다운 옵션과
`#/stats` 회차별 정답률 줄에 `· N회 풀이` 로 표시.
