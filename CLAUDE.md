...
- **저장 탭** `#/saved`(하단 탭 ⭐): 즐겨찾기 / 메모 2-세그먼트. 두 목록 모두 `reviewItem` 펼침 행 —
  ...
  이리로 옮겼고, `더보기` 상단엔 저장·지난 기록·통계로 가는 `.menu-row` 바로가기만 남김.
...
- 진행 데이터는 기기별 localStorage. 더보기 > 내보내기/가져오기(JSON)로 기기 간 이동. 앱 셸(index.html/style.css/app.js/sw.js) 변경 시 `docs/sw.js` `CACHE` 버전을 올린다 (현재 **v23**).
